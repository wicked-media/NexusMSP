...
const EXPERIENCE_IDS = ["classic", "constellation", "theatre", "calm", "hero-monogram", "orbital-signature"];
...
export default function LoginPage() {
  ...
  const isNexusBrand = !brand.company_logo_url && (brand.company_name || "NexusMSP").replace(/\s/g, "").toLowerCase() === "nexusmsp";
  const isPremiumNexusExperience = isNexusBrand && ["hero-monogram", "orbital-signature"].includes(experience);
  ...
  return (
    <div ref={loginRootRef} className={`login-experience nexus-signin login-experience-${experience} relative flex min-h-[100svh] overflow-x-hidden bg-[#020611] lg:h-[100svh] lg:min-h-[680px] lg:overflow-hidden`} data-testid="login-page" data-login-experience={experience}>
      ...
      <div className={`login-story-panel nexus-login-stage hidden lg:flex relative z-10 ${experience === "classic" ? "lg:w-[57%]" : isPremiumNexusExperience ? "lg:w-[63%]" : experience === "calm" ? "lg:w-[65%]" : "lg:w-[64%]"}`}>
        <div className="relative flex w-full flex-col justify-between overflow-hidden p-10 xl:p-14 2xl:p-16">
          ...
          {isPremiumNexusExperience && (
            <div className={`nexus-premium-stage nexus-premium-stage-${experience}`} aria-label={`${experience === "hero-monogram" ? "Hero Monogram" : "Orbital Signature"} Nexus MSP login experience`}>
              ...
              <img src={`/login-experiences/${experience}.png`} alt="" aria-hidden="true" className="nexus-premium-neon" />
      ...
      <style>{`
        ...
        .nexus-premium-neon { position:absolute; inset:0; width:100%; height:100%; padding:clamp(.25rem,.9vw,1rem) clamp(1.75rem,4vw,5rem); object-fit:contain; object-position:50% 50%; pointer-events:none; mix-blend-mode:screen; opacity:.18; filter:contrast(1.35) saturate(2.3) brightness(1.5) drop-shadow(0 0 8px rgba(17,205,255,.88)) drop-shadow(0 0 22px rgba(31,134,255,.46)); animation:nexusPremiumNeonDrift 18s cubic-bezier(.45,.05,.55,.95) infinite alternate, nexusPremiumNeon 4.8s ease-in-out infinite; will-change:transform,opacity,filter; }
        .nexus-premium-stage-hero-monogram .nexus-premium-art { animation-duration:22s; }
        .nexus-premium-stage-hero-monogram .nexus-premium-neon { animation-duration:22s,5.6s; }
        ...
        @keyframes nexusPremiumNeon { 0%,100% { opacity:.12; filter:contrast(1.25) saturate(1.8) brightness(1.28) drop-shadow(0 0 5px rgba(17,205,255,.5)); } 42% { opacity:.34; filter:contrast(1.45) saturate(2.8) brightness(1.8) drop-shadow(0 0 10px rgba(17,205,255,.98)) drop-shadow(0 0 28px rgba(31,134,255,.6)); } 58% { opacity:.24; filter:contrast(1.35) saturate(2.2) brightness(1.5) drop-shadow(0 0 8px rgba(17,205,255,.78)) drop-shadow(0 0 20px rgba(31,134,255,.4)); } }
