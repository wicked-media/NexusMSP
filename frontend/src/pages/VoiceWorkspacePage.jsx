...
import {
  ...
  Loader2, Phone, Plus, RefreshCw, Search, Settings, ShieldCheck, Users, Wifi, WifiOff,
} from "lucide-react";

const VOICE_TABS = ["dashboard", "pbxs", "extensions", "billing", "sync", "activity", "diagnostics", "settings"];
const emptyPbxForm = {
...
export default function VoiceWorkspacePage() {
  ...
  const addPbx = async () => {
    if (!pbxForm.client_id || !pbxForm.name.trim() || !pbxForm.pbx_url.trim()) {
      toast.error("Choose a client and enter the PBX name and Cloud URL");
      return;
    }
    setBusy("add-pbx");
    try {
      await axios.post(`${API}/yeastar/pbxs`, pbxForm, { headers });
      toast.success("Yeastar PBX linked to the client");
      setShowAddPbx(false);
  ...
  return <div className="space-y-5" data-testid="voice-workspace-page">
    <OperationalPageHeader
      ...
      tone="sky"
      actions={<>
        <Button variant="outline" size="sm" onClick={() => navigate("/settings?tab=integrations&anchor=voice-services-settings-card")} disabled={!!busy} data-testid="voice-open-provider-settings"><Settings className="mr-1.5 h-3.5 w-3.5" />Provider setup</Button>
        ...
        <Button size="sm" onClick={() => setShowAddPbx(true)} disabled={!!busy} data-testid="voice-add-pbx"><Plus className="mr-1.5 h-3.5 w-3.5" />Add PBX</Button>
      </>}
    />
    ...
    <Card className="overflow-hidden border-cyan-500/20 bg-[linear-gradient(135deg,rgba(8,47,73,0.18),rgba(15,23,42,0.42))]" data-testid="voice-architecture-card">
      <CardContent className="grid gap-4 p-4 lg:grid-cols-[1.2fr_repeat(3,minmax(0,1fr))] lg:items-center">
        <div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-cyan-300">Provider-to-client workflow</p><p className="mt-1 text-sm text-muted-foreground">YCM is a provider control-plane. Client PBXs remain isolated so credentials, extensions, billing, and audit history never cross accounts.</p></div>
        {[['1', 'Provider setup', 'Review the Yeastar/YCM entry in Settings.'], ['2', 'Link PBX', 'Choose the client before saving its PBX API details.'], ['3', 'Sync & bill', 'Use live extension counts, product mapping, and recurring billing.']].map(([number, title, copy]) => <div className="flex gap-3" key={number}><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-cyan-400/30 bg-cyan-400/10 text-xs font-semibold text-cyan-200">{number}</span><div><p className="text-sm font-medium">{title}</p><p className="mt-0.5 text-xs text-muted-foreground">{copy}</p></div></div>)}
      </CardContent>
    </Card>
    ...
    <Tabs value={tab} onValueChange={(next) => updateRoute(next)}>
      <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-xl border border-border/50 bg-card/70 p-1.5 sm:w-fit">
        <TabsTrigger value="dashboard">Dashboard</TabsTrigger><TabsTrigger value="pbxs">PBXs ({pbxs.length})</TabsTrigger><TabsTrigger value="extensions">Extensions ({scopedExtensions.length})</TabsTrigger><TabsTrigger value="billing">Billing</TabsTrigger><TabsTrigger value="sync">Sync history</TabsTrigger><TabsTrigger value="activity">Activity</TabsTrigger><TabsTrigger value="diagnostics">API diagnostics</TabsTrigger><TabsTrigger value="settings">Provider setup</TabsTrigger>
      </TabsList>

      <TabsContent value="dashboard" className="mt-4 grid gap-4 lg:grid-cols-3">
        <Card className="border-sky-500/20 lg:col-span-2"><CardHeader><CardTitle className="text-sm">Provider health</CardTitle><CardDescription>Connection readiness is measured from client-owned PBX records, not a shared credential.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 bg-background/40 p-4"><div className="flex items-center gap-3"><div className={`flex h-10 w-10 items-center justify-center rounded-lg ${workspace.provider?.connected ? "bg-emerald-500/10" : "bg-amber-500/10"}`}><Cloud className={`h-5 w-5 ${workspace.provider?.connected ? "text-emerald-300" : "text-amber-300"}`} /></div><div><p className="font-semibold">Yeastar / YCM</p><p className="text-xs text-muted-foreground">{workspace.provider?.connected ? "Client PBX connections are ready for live checks" : "Add a client PBX to begin managed voice operations"}</p></div></div><Badge variant="outline" className={workspace.provider?.connected ? "border-emerald-500/30 text-emerald-300" : "border-amber-500/30 text-amber-300"}>{readable(workspace.system_health)}</Badge></div><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-lg border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Last successful sync</p><p className="mt-1 text-sm font-medium">{workspace.last_successful_sync ? compactDate(workspace.last_successful_sync) : "Not yet synchronised"}</p></div><div className="rounded-lg border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Client linkage</p><p className="mt-1 text-sm font-medium">{allPbxs.length} client PBX{allPbxs.length === 1 ? "" : "s"}</p></div><div className="rounded-lg border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Billing review</p><p className="mt-1 text-sm font-medium">{(workspace.billing?.pending_changes || 0)} change{(workspace.billing?.pending_changes || 0) === 1 ? "" : "s"} pending</p></div></div></CardContent></Card>
        <Card><CardHeader><CardTitle className="text-sm">Recent activity</CardTitle></CardHeader><CardContent className="space-y-3">{(workspace.sync_history || []).slice(0, 4).map((entry) => <div className="flex gap-2 text-xs" key={entry.id}><Activity className={`mt-0.5 h-3.5 w-3.5 ${entry.status === "success" ? "text-emerald-300" : "text-rose-300"}`} /><div><p className="font-medium">{entry.pbx_name || "Yeastar PBX"} · {readable(entry.status)}</p><p className="text-muted-foreground">{entry.completed_at ? compactDate(entry.completed_at) : "In progress"}</p></div></div>)}{!workspace.sync_history?.length && <div className="space-y-3 py-1"><p className="text-sm text-muted-foreground">No PBX activity yet.</p><Button size="sm" className="w-full" onClick={() => setShowAddPbx(true)}><Plus className="mr-2 h-3.5 w-3.5" />Add first client PBX</Button></div>}</CardContent></Card>
      </TabsContent>
      ...
      <TabsContent value="pbxs" className="mt-4 space-y-3">
        ...
        <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Customer / PBX</TableHead><TableHead>Cloud URL</TableHead><TableHead>Status</TableHead><TableHead>Extensions</TableHead><TableHead>Billing</TableHead><TableHead>Last sync</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{pbxs.map((pbx) => <TableRow key={pbx.id}><TableCell><button className="text-left" onClick={() => navigate(`/clients?client=${encodeURIComponent(pbx.client_id)}`)}><p className="font-medium hover:text-cyan-300">{pbx.client_name}</p><p className="text-xs text-muted-foreground">{pbx.name}</p></button></TableCell><TableCell className="max-w-56 truncate font-mono text-xs">{pbx.url || "Not configured"}</TableCell><TableCell><StatusBadge status={pbx.status} /></TableCell><TableCell>{pbx.extension_count || 0}</TableCell><TableCell>{pbx.billable_extension_count || 0} billable</TableCell><TableCell className="text-xs">{compactDate(pbx.last_sync)}</TableCell><TableCell><div className="flex justify-end gap-1"><Button variant="ghost" size="sm" onClick={() => testConnection(pbx.id)} disabled={!!busy || !pbx.has_credentials}><Wifi className="mr-1 h-3.5 w-3.5" />Test</Button><Button variant="outline" size="sm" onClick={() => runSync(pbx.id)} disabled={!!busy || !pbx.has_credentials}><RefreshCw className={`mr-1 h-3.5 w-3.5 ${busy === `sync-${pbx.id}` ? "animate-spin" : ""}`} />Sync</Button><Button variant="ghost" size="sm" onClick={() => openPbxSettings(pbx)} disabled={!!busy}><Settings className="mr-1 h-3.5 w-3.5" />Edit</Button></div></TableCell></TableRow>)}{!pbxs.length && <TableRow><TableCell colSpan={7} className="py-12 text-center"><p className="text-sm text-muted-foreground">No PBXs are linked in this view.</p><Button size="sm" className="mt-3" onClick={() => setShowAddPbx(true)}><Plus className="mr-2 h-3.5 w-3.5" />Link a client PBX</Button></TableCell></TableRow>}</TableBody></Table></CardContent></Card>
      </TabsContent>
      ...
      <TabsContent value="settings" className="mt-4 grid gap-4 lg:grid-cols-2"><Card className="border-cyan-500/20"><CardHeader><CardTitle className="text-sm">Yeastar / YCM provider routing</CardTitle><CardDescription>Provider-level setup belongs in Settings. The client-first PBX model stays in Voice.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="rounded-xl border border-cyan-500/20 bg-cyan-500/[0.05] p-4"><p className="font-medium">Why YCM is not the billing record</p><p className="mt-1 text-sm text-muted-foreground">A central provider connection is useful for administration, but a single YCM secret must never decide which client receives an extension bill. Each PBX record holds its own client link, API credentials, product mapping, and audit trail.</p></div><Button onClick={() => navigate("/settings?tab=integrations&anchor=voice-services-settings-card")}><Settings className="mr-2 h-4 w-4" />Open provider settings</Button></CardContent></Card><Card><CardHeader><CardTitle className="text-sm">Client PBX configuration</CardTitle><CardDescription>Set these values on the PBX record so every connection remains account-scoped.</CardDescription></CardHeader><CardContent className="space-y-3">{[["Client link", "Identifies the account where the PBX status and activity appear."], ["P-Series API credentials", "Used only to query that customer’s PBX endpoint."], ["Product mapping", "Required before live extension quantities can attach to recurring billing."]].map(([title, copy]) => <div className="flex gap-3 rounded-lg border border-border/60 p-3" key={title}><ShieldCheck className="mt-0.5 h-4 w-4 text-emerald-300" /><div><p className="text-sm font-medium">{title}</p><p className="mt-0.5 text-xs text-muted-foreground">{copy}</p></div></div>)}<Button variant="outline" onClick={() => setShowAddPbx(true)}><Plus className="mr-2 h-4 w-4" />Add client PBX</Button></CardContent></Card></TabsContent>
    </Tabs>
    ...
    <Dialog open={showAddPbx} onOpenChange={setShowAddPbx}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Link a Yeastar PBX</DialogTitle><DialogDescription>Choose the client first. The PBX will appear on the account profile, its extensions will remain client-scoped, and billing will use that client’s recurring invoice.</DialogDescription></DialogHeader><SetupGuideCallout title="Where do these credentials come from?" source="In the customer’s Yeastar P-Series PBX portal, enable Integrations → API, then copy the Client ID and Client Secret. Enter the base PBX URL only—NexusMSP adds the OpenAPI path." securityNote="Store the Client Secret only in NexusMSP; never put it in a ticket, client note, or chat." helpSlug="voice-yeastar-pbx-onboarding" /><PbxForm form={pbxForm} setForm={setPbxForm} clients={clients} products={products} /><DialogFooter><Button variant="ghost" onClick={() => setShowAddPbx(false)}>Cancel</Button><Button onClick={addPbx} disabled={busy === "add-pbx"}>{busy === "add-pbx" && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Link PBX to client</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}
...
function PbxForm({ form, setForm, clients, products = [], editing = false }) {
  ...
  const productLabel = (product) => `${product.name || "Unnamed product"}${product.sku ? ` · ${product.sku}` : ""}`;
  ...
  const productDisplayValue = productSearch || (selectedProduct ? productLabel(selectedProduct) : form.product_mapping || "");

  return <div className="space-y-4 voice-pbx-form">
    <div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1.5"><Label>Client</Label><Select value={form.client_id || ""} onValueChange={(client_id) => update({ client_id })}><SelectTrigger><SelectValue placeholder="Choose client" /></SelectTrigger><SelectContent>{clients.map((client) => <SelectItem key={client.id} value={client.id}>{client.name}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1.5"><Label>PBX name</Label><Input value={form.name || ""} onChange={(event) => update({ name: event.target.value })} placeholder="Main office PBX" /></div></div>
    <div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1.5"><Label>Yeastar Cloud URL / FQDN</Label><Input value={form.pbx_url || ""} onChange={(event) => update({ pbx_url: event.target.value })} placeholder="https://pbx.customer.example" /></div><div className="space-y-1.5"><Label>Sync schedule</Label><Select value={form.auto_sync_schedule || "daily"} onValueChange={(auto_sync_schedule) => update({ auto_sync_schedule })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="hourly">Hourly</SelectItem><SelectItem value="daily">Daily</SelectItem><SelectItem value="weekly">Weekly</SelectItem></SelectContent></Select></div></div>
    <div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1.5"><Label>Client ID</Label><Input value={form.client_api_id || ""} onChange={(event) => update({ client_api_id: event.target.value })} placeholder="Yeastar API Client ID" /></div><div className="space-y-1.5"><Label>Client Secret</Label><Input type="password" value={form.client_secret || ""} onChange={(event) => update({ client_secret: event.target.value })} placeholder={editing ? "Leave blank to retain" : "Yeastar API Client Secret"} /></div></div>
    ...
    <div className="space-y-1.5"><Label>Recurring billing product</Label><div className="relative"><Input value={productDisplayValue} onChange={(event) => { setProductSearch(event.target.value); update({ product_mapping: "" }); }} placeholder="Search by product name, SKU, or product ID" data-testid="voice-product-autocomplete" />{productSearch && <div className="absolute z-30 mt-1 max-h-56 w-full overflow-auto rounded-lg border border-border bg-popover p-1 shadow-xl">{matchingProducts.length ? matchingProducts.map((product) => <button type="button" key={product.id} className="flex w-full items-center justify-between gap-3 rounded-md px-3 py-2 text-left text-sm hover:bg-muted" onClick={() => { update({ product_mapping: product.id }); setProductSearch(""); }}><span className="min-w-0 truncate font-medium">{productLabel(product)}</span><span className="shrink-0 text-xs text-muted-foreground">{productRate(product) > 0 ? `$${productRate(product).toFixed(2)}` : "No price"}</span></button>) : <p className="px-3 py-2 text-xs text-muted-foreground">No NexusMSP product matches that search.</p>}</div>}</div>{selectedProduct ? <p className={`text-xs ${productRate(selectedProduct) > 0 ? "text-emerald-300" : "text-amber-300"}`}>Mapped to {productLabel(selectedProduct)}{productRate(selectedProduct) > 0 ? ` at $${productRate(selectedProduct).toFixed(2)} per extension` : " — add a unit price before enabling recurring billing."}</p> : <p className="text-xs text-muted-foreground">Search and choose the billable NexusMSP product. A priced product is required before live extension quantities can attach to recurring billing.</p>}</div>
