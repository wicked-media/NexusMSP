...
export default function VoiceWorkspacePage() {
  ...
  useEffect(() => {
    ...
    loadMonitoring(monitoringPbxId);
    const timer = window.setInterval(() => loadMonitoring(monitoringPbxId), 10000);
  ...
  return <div className="space-y-5" data-testid="voice-workspace-page">
    ...
    <Tabs value={tab} onValueChange={(next) => updateRoute(next)}>
      ...
      <TabsContent value="monitoring" className="mt-4 space-y-4">
        <Card className="border-cyan-500/20"><CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-end"><div className="min-w-0 flex-1 space-y-1.5"><Label>PBX to monitor</Label><Select value={monitoringPbxId} onValueChange={setMonitoringPbxId}><SelectTrigger><SelectValue placeholder="Choose a client PBX" /></SelectTrigger><SelectContent><SelectItem value="all">All authorised client PBXs</SelectItem>{pbxs.map((pbx) => <SelectItem key={pbx.id} value={pbx.id}>{pbx.client_name} · {pbx.name}</SelectItem>)}</SelectContent></Select></div><Button onClick={() => loadMonitoring()} disabled={!monitoringPbxId || !!busy} data-testid="voice-run-monitoring"><Activity className={`mr-2 h-4 w-4 ${busy === "monitoring" ? "animate-pulse" : ""}`} />{busy === "monitoring" ? "Checking…" : monitoringPbxId === "all" ? "Open wallboard" : "Run live check"}</Button></CardContent></Card>
