...
export default function VoiceWorkspacePage() {
  ...
  return <div className="space-y-5" data-testid="voice-workspace-page">
    ...
    <Dialog open={!!editingPbx} onOpenChange={(open) => { if (!open) setEditingPbx(null); }}><DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>PBX configuration</DialogTitle><DialogDescription>Maintain one customer connection, billing mapping, and safeguards. Leave Client Secret blank to retain the securely stored value.</DialogDescription></DialogHeader><div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3"><p className="text-sm font-medium text-cyan-100">Connection-safe editing</p><p className="mt-1 text-xs text-muted-foreground">NexusMSP tests the effective URL, API identity, stored or rotated secret, and TLS policy before any edited configuration is committed.</p></div>{editingPbx && <PbxForm form={editingPbx} setForm={setEditingPbx} clients={clients} products={products} editing />}<DialogFooter><Button variant="ghost" onClick={() => setEditingPbx(null)}>Cancel</Button><Button onClick={savePbxSettings} disabled={busy === "save-pbx"}>{busy === "save-pbx" && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{busy === "save-pbx" ? "Testing connection…" : "Test & save configuration"}</Button></DialogFooter></DialogContent></Dialog>
