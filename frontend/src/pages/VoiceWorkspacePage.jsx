  ...
  return <div className="space-y-5" data-testid="voice-workspace-page">
    ...
    <Tabs value={tab} onValueChange={(next) => updateRoute(next)}>
      ...
      <TabsContent value="monitoring" className="mt-4 space-y-4">
        ...
        {!monitoring ? <Card><CardContent className="py-14 text-center"><Activity className="mx-auto mb-3 h-9 w-9 text-cyan-300/50" /><p className="text-sm font-medium">Choose a PBX to see its live operational state</p><p className="mt-1 text-xs text-muted-foreground">Nexus checks the PBX API, active calls, extension registration and recent call outcomes without exposing credentials.</p></CardContent></Card> : <>
