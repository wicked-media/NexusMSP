...
import { MessageCircle, X, Send, Hash, Users, ChevronUp } from "lucide-react";
...
export function ChatPanel() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!alive) return;
        setChannels(chs); setPresence(pr.users || []); setUnread(un);
  ...
  useEffect(() => {
    const onKey = (e) => {
      if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === SHORTCUT && !["INPUT", "TEXTAREA"].includes(e.target.tagName)) {
        e.preventDefault();
        setOpen(v => !v);
      }
    };
    ...
    <div className="fixed bottom-20 right-4 z-50 w-[640px] max-w-[95vw] h-[560px] max-h-[85vh] bg-background border border-border rounded-lg shadow-2xl shadow-violet-500/10 flex overflow-hidden" data-testid="chat-panel">
      ...
      <div className="w-44 border-r border-border bg-muted/20 flex flex-col">
        <div className="p-3 border-b border-border flex items-center justify-between">
          <div className="text-xs font-semibold uppercase tracking-widest text-violet-400">Chat</div>
        ...
        <ScrollArea className="flex-1">
          ...
          {channels.filter(c => c.kind === "dm").map(c => (
            <button key={c.id}
              ...
              className={`w-full text-left px-3 py-1.5 text-xs flex items-center gap-2 ${activeId === c.id ? "bg-violet-500/10 text-violet-400" : "hover:bg-muted/40"}`}
            >
              <Users className="w-3 h-3" />{c.display_name || c.name.replace(/^dm:/, "").substring(0, 12)}
              {unread[c.id] > 0 && <Badge variant="outline" className="ml-auto text-[9px] text-rose-400 border-rose-500/40 px-1">{unread[c.id]}</Badge>}
            </button>
          ))}
          {showDirectory && (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="px-3 py-2 border-b border-border flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" className="h-6 px-2" onClick={() => setOpen(false)} data-testid="chat-close-btn">
