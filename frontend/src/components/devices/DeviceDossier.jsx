...
function HealthGauge({ score }) {
  ...
  return <svg width="96" height="96" viewBox="0 0 96 96"><circle cx="48" cy="48" r={radius} stroke="hsl(var(--border))" strokeWidth="8" fill="none" /><circle cx="48" cy="48" r={radius} stroke={tone} strokeWidth="8" fill="none" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" transform="rotate(-90 48 48)" style={{ transition: "stroke-dashoffset 0.6s ease" }} /><text x="48" y="52" textAnchor="middle" fontSize="22" fontWeight="700" fill="hsl(var(--foreground))" fontFamily="monospace">{score}</text><text x="48" y="68" textAnchor="middle" fontSize="9" fill="hsl(var(--muted-foreground))" fontFamily="monospace">HEALTH</text></svg>;
}
...
export default function DeviceDossier({ deviceId, headers, API }) {
  ...
  return <Card className="border-cyan-500/20 bg-gradient-to-br from-card via-card to-cyan-500/[0.04]" data-testid="device-dossier">
    <CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle className="flex items-center gap-2 text-sm"><Sparkles className="h-4 w-4 text-cyan-600 dark:text-cyan-300" />Intelligence Dossier</CardTitle><Button variant="ghost" size="sm" className="h-7 text-[10px]" onClick={load}><RefreshCw className="mr-1 h-3 w-3" />Refresh</Button></CardHeader>
