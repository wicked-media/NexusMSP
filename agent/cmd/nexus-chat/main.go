...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/elevate/progress" {
			_, _ = io.WriteString(w, elevationProgressPageV2)
			return
		}
...
func elevationRequestProxy(cfg *config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.NewDecoder(io.LimitReader(r.Body, 32*1024)).Decode(&form); err != nil {
			writeJSONError(w, "Invalid elevation request", http.StatusBadRequest)
			return
		}
		...
		if !filepath.IsAbs(programPath) || !strings.EqualFold(filepath.Ext(programPath), ".exe") {
			writeJSONError(w, "Choose an absolute Windows .exe path", http.StatusBadRequest)
			return
		}
		if len(strings.TrimSpace(form.Justification)) < 8 {
			writeJSONError(w, "Explain why administrator access is required", http.StatusBadRequest)
			return
		}
		hash, err := sha256File(programPath)
		if err != nil {
			writeJSONError(w, "Unable to fingerprint the selected program: "+err.Error(), http.StatusBadRequest)
			return
		}
		...
		if len(arguments) > 64 {
			writeJSONError(w, "Too many arguments", http.StatusBadRequest)
			return
		}
...
func writeJSONError(w http.ResponseWriter, detail string, status int) {
	w.Header().Set("Content-Type", "application/json")
...
const elevationProgressPageV2 = `<!doctype html><html><head><meta charset="utf-8"><title>Nexus Elevate Progress</title><style>body{margin:0;background:radial-gradient(circle at top right,#065f4650,transparent 36%),#10151d;color:#eef2f7;font:14px Segoe UI,Arial,sans-serif}.wrap{width:min(860px,calc(100% - 32px));margin:auto;padding:34px 0}.back{color:#a5f3fc;text-decoration:none;font-size:12px}.card{margin-top:18px;border:1px solid #ffffff16;border-radius:18px;background:#151b24;padding:24px;box-shadow:0 18px 60px #0005}.eyebrow{font-size:10px;color:#67e8f9;font-weight:700;letter-spacing:.16em}.title{font-size:25px;margin:5px 0}.sub{color:#94a3b8;line-height:1.55}.row{display:grid;grid-template-columns:1fr auto;gap:12px;padding:14px 0;border-bottom:1px solid #ffffff12}.row:last-child{border:0}.meta{font-size:11px;color:#94a3b8;margin-top:4px}.pill{font-size:10px;text-transform:uppercase;font-weight:700;letter-spacing:.08em;border:1px solid #ffffff20;padding:5px 8px;border-radius:999px;height:max-content}.pending{color:#fde68a;background:#713f1244}.approved{color:#bae6fd;background:#0c4a6e55}.executed{color:#bbf7d0;background:#14532d55}.denied,.failed,.expired{color:#fecdd3;background:#4c051955}.empty{color:#94a3b8;padding:24px 0}</style></head><body><main class="wrap"><a class="back" href="/">← Return to client chat</a><section class="card"><div class="eyebrow">NEXUS ELEVATE · ENDPOINT REQUESTS</div><h1 class="title">Request progress</h1><p class="sub">See controlled-access requests made from this device. Support reviews the exact program, fingerprint and reason before anything can run.</p><div id="list" class="empty">Loading protected request history…</div></section></main><script>const list=document.querySelector('#list');const esc=s=>{const x=document.createElement('span');x.textContent=s||'';return x.innerHTML};async function load(){try{const r=await fetch('/api/elevate/recent');const d=await r.json();if(!r.ok)throw Error(d.detail||'History unavailable');const rows=d.requests||[];list.innerHTML=rows.length?rows.map(x=>'<article class="row"><div><strong>'+esc(x.program_name||'Approved executable')+'</strong><div class="meta">'+esc((x.requested_at||'').replace('T',' ').replace('Z',''))+(x.denial_reason?' · '+esc(x.denial_reason):'')+'</div></div><span class="pill '+esc(x.status||'pending')+'">'+esc((x.status||'pending').replace('_',' '))+'</span></article>').join(''):'<div class="empty">No controlled-access requests have been made from this device.</div>'}catch(e){list.innerHTML='<div class="empty">'+esc(e.message)+'</div>'}}load();setInterval(load,5000)</script></body></html>`
...
const elevationProgressPage = `<!doctype html><html><head><meta charset="utf-8"><title>Nexus Elevate Progress</title><style>*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top right,#065f4650,transparent 36%%),#10151d;color:#eef2f7;font:14px Segoe UI,Arial,sans-serif}.wrap{width:min(860px,calc(100%% - 32px));margin:auto;padding:34px 0}.back{color:#a5f3fc;text-decoration:none;font-size:12px}.card{margin-top:18px;border:1px solid #ffffff16;border-radius:18px;background:#151b24;padding:24px;box-shadow:0 18px 60px #0005}.eyebrow{font-size:10px;color:#67e8f9;font-weight:700;letter-spacing:.16em}.title{font-size:25px;margin:5px 0}.sub{color:#94a3b8;line-height:1.55}.row{display:grid;grid-template-columns:1fr auto;gap:12px;padding:14px 0;border-bottom:1px solid #ffffff12}.row:last-child{border:0}.meta{font-size:11px;color:#94a3b8;margin-top:4px}.pill{font-size:10px;text-transform:uppercase;font-weight:700;letter-spacing:.08em;border:1px solid #ffffff20;padding:5px 8px;border-radius:999px;height:max-content}.pending{color:#fde68a;background:#713f1244}.approved{color:#bae6fd;background:#0c4a6e55}.executed{color:#bbf7d0;background:#14532d55}.denied,.failed,.expired{color:#fecdd3;background:#4c051955}.empty{color:#94a3b8;padding:24px 0}</style></head><body><main class="wrap"><a class="back" href="/">← Return to client chat</a><section class="card"><div class="eyebrow">NEXUS ELEVATE · ENDPOINT REQUESTS</div><h1 class="title">Request progress</h1><p class="sub">See the controlled-access requests made from this device. Your support team reviews exact program, fingerprint and business reason before anything can run.</p><div id="list" class="empty">Loading protected request history…</div></section></main><script>const list=document.querySelector('#list');const esc=s=>{const x=document.createElement('span');x.textContent=s||'';return x.innerHTML};async function load(){try{const r=await fetch('/api/elevate/recent');const d=await r.json();if(!r.ok)throw Error(d.detail||'History unavailable');const rows=d.requests||[];list.innerHTML=rows.length?rows.map(x=>'<article class="row"><div><strong>'+esc(x.program_name||'Approved executable')+'</strong><div class="meta">'+esc((x.requested_at||'').replace('T',' ').replace('Z',''))+(x.denial_reason?' · '+esc(x.denial_reason):'')+'</div></div><span class="pill '+esc(x.status||'pending')+'">'+esc((x.status||'pending').replace('_',' '))+'</span></article>').join(''):'<div class="empty">No controlled-access requests have been made from this device.</div>'}catch(e){list.innerHTML='<div class="empty">'+esc(e.message)+'</div>'}}load();setInterval(load,5000)</script></body></html>`
