...
def test_web_studio_overview_is_limited_to_the_technicians_clients(monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr(web_studio, "db", type("WebStudioDB", (), {"web_sites": Sites()})())
    user = {"id": "tech-1", "role": "technician", "client_scope_mode": "restricted", "client_scope_ids": ["client-a"]}

    response = asyncio.run(web_studio.get_web_studio_overview(user=user))

    assert response["sites"] == []
    assert captured["query"] == {"$and": [{"archived_at": {"$exists": False}}, {"client_id": {"$in": ["client-a"]}}]}


def test_restricted_technician_cannot_update_foreign_web_site(monkeypatch):
    denials = _InsertCollection()
    monkeypatch.setattr(scope_permissions.db, "scope_denials", denials)
    monkeypatch.setattr(web_studio, "db", type("WebStudioDB", (), {"web_sites": _RecordCollection({"id": "site-b", "client_id": "client-b"})})())
    user = {"id": "tech-1", "role": "technician", "client_scope_mode": "restricted", "client_scope_ids": ["client-a"]}

    with pytest.raises(HTTPException) as exc:
        asyncio.run(web_studio.update_web_site("site-b", web_studio.WebSiteUpdate(name="Changed"), user))
