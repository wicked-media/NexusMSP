"""
Hudu integration — feature-rich read-only client.
Fixes: correct 'search' query param, paginated fetch of all resource types,
AI-powered KB suggestions for tickets via Emergent LLM key (Claude Sonnet 4.5).
"""
import os
import re
import uuid
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0, verify=verify_ssl) as client:
            ...
            if r.status_code == 401:
                raise HTTPException(401, "Hudu authentication failed — check API key")
            if r.status_code == 403:
                # Endpoint disabled on plan — return empty, not an error
                return None
            if r.status_code == 429:
                raise HTTPException(429, "Hudu rate-limit hit — slow down")
...
def _ok_config() -> dict:
    ...
    return {}  # placeholder — we check config async in each route


def _not_configured():
    return HTTPException(503, "Hudu not configured — add URL and API key in Settings")
...
async def test_hudu_connection(current_user: dict = Depends(get_current_user)):
    ...
    if not config:
        return {"success": False, "message": "Not configured — enter URL and API key first"}
    now = datetime.now(timezone.utc).isoformat()
    try:
        ...
        if api_key:
            try:
                ...
                prompt = (
                    ...
                    "Pick the 1-3 most relevant items. For each, give: id, why it's relevant (≤1 sentence), "
                    "and a concrete suggested fix (≤3 bullet points) distilled from the content. "
