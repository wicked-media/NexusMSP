...
MICROSOFT_ACTION_TEMPLATES: tuple[dict, ...] = (
    ...
    {
        "id": "manage-group-access",
        "label": "Manage group access",
        "description": "Add or remove a user from a Microsoft group with a tenant, client and approval trail.",
        "permission": "entra.group.modify",
        "impact": "high",
        "approval_required": True,
        "target": "user",
        "fields": [
            {
                ...
                "label": "Group name or object ID",
                "type": "text",
                "required": True,
                "placeholder": "Finance-Approvers or group GUID",
            },
            {
                "key": "membership_operation",
                "label": "Membership operation",
                "type": "select",
                "required": True,
                "default": "add",
                "options": [
            ...
            },
            {
                ...
                "options": [
                    {"value": "manager_confirmed", "label": "Manager or service owner confirmed"},
                    {"value": "ticket_authorised", "label": "Authorised in linked ticket or change"},
                ],
            },
        ],
        "rollback": "Restore the recorded before-state by reversing the group membership change after the access owner confirms the correction.",
        "steps": [
            "Confirm the tenant, client, user and target group",
            "Record the access owner authority and service reference",
            "Request approval for the membership delta",
            "Apply the approved change through the authorised provider and retain before/after evidence",
        ],
    },
    {
        "id": "manage-privileged-role",
        "label": "Manage privileged role",
        "description": "Assign, remove or time-bound an Entra directory role through a separately approved access plan.",
        "permission": "entra.role.modify",
        "impact": "critical",
        "approval_required": True,
        "target": "user",
        "fields": [
            {
                ...
                "type": "text",
                "required": True,
                "placeholder": "Exchange Administrator or role definition ID",
            },
            {
                "key": "role_operation",
                ...
                "type": "select",
                "required": True,
                "default": "assign_eligible",
                "options": [
                    {"value": "assign_eligible", "label": "Assign eligible role"},
                    {"value": "activate_time_bound", "label": "Activate time-bound role"},
                    {"value": "remove", "label": "Remove role"},
                ],
            },
            {
                "key": "access_duration", "label": "Maximum access duration", "type": "select", "required": True, "default": "8_hours",
                "options": [
                    {"value": "1_hour", "label": "1 hour"}, {"value": "8_hours", "label": "8 hours"},
                    {"value": "24_hours", "label": "24 hours"}, {"value": "permanent_reviewed", "label": "Permanent (separate review)"},
                ],
            },
            {
                "key": "role_owner", "label": "Role owner or approval authority", "type": "text", "required": True,
                "placeholder": "Security owner, manager or CAB reference",
            },
        ],
        "rollback": "Remove the role assignment or end the active role session, then retain the before/after directory-role evidence with the approval record.",
        "steps": [
            "Confirm the user, tenant, client and requested directory role",
            "Validate the least-privilege duration and named access owner",
            "Request independent approval with service or change context",
            "Apply only through an authorised provider and retain assignment evidence",
        ],
    },
    {
        "id": "manage-mailbox-access",
        "label": "Manage mailbox access",
        "description": "Grant, change or remove a user's shared-mailbox access through an approved and auditable plan.",
        "permission": "exchange.mailbox.delegate",
        "impact": "high",
        "approval_required": True,
        "target": "user",
        "fields": [
            ...
            {
                "key": "delegation_operation",
                "label": "Delegation operation",
                "type": "select",
                "required": True,
                "default": "grant_full_access",
                "options": [
                    {"value": "grant_full_access", "label": "Grant full access"},
                    {"value": "grant_send_as", "label": "Grant Send As"},
                    {"value": "grant_send_on_behalf", "label": "Grant Send on behalf"},
                    {"value": "remove_delegate", "label": "Remove delegate access"},
                ],
            },
            {
                "key": "access_owner",
                "label": "Mailbox owner or approval authority",
                "type": "text",
                "required": True,
                "placeholder": "Finance manager or service owner",
            },
        ],
        "rollback": "Remove the recorded delegation permission and verify the mailbox ACL matches its retained before-state.",
        "steps": [
            "Confirm the tenant, client, mailbox and target user",
            "Record the mailbox owner authority and service reference",
            "Request approval for the delegation delta",
            "Apply the approved change through the authorised provider and retain mailbox permission evidence",
        ],
    },
    {
        "id": "retire-managed-device",
        "label": "Retire managed device",
        "description": "Retire, wipe or remove an Intune-managed device through a tenant-scoped, approval-required plan.",
        "permission": "intune.device.retire",
        "impact": "critical",
        "approval_required": True,
        "target": "device",
        "fields": [
            {
                ...
                "options": [
                    ...
                    {"value": "remove_record", "label": "Remove Intune record only"},
                ],
            },
            {
                "key": "recovery_confirmed",
                "label": "Recovery or handover evidence",
                "type": "select",
                "required": True,
                "default": "ticket_confirmed",
                "options": [
                    {"value": "ticket_confirmed", "label": "Confirmed in service ticket"},
                    {"value": "manager_confirmed", "label": "Confirmed by client manager"},
                    {"value": "security_incident", "label": "Authorised security incident"},
                ],
            },
        ],
        "rollback": "A completed wipe cannot be reversed. For retire or record removal, re-enrol the device only after confirming ownership, compliance and data-protection requirements.",
        "steps": [
            "Confirm the tenant, client and exact Intune managed-device identifier",
            "Record recovery, handover or incident authority",
            "Request approval with the linked ticket or change reference",
            "Apply the approved provider action and retain result evidence",
        ],
    },
    {
        "id": "manage-conditional-access",
        "label": "Manage Conditional Access",
        "description": "Create, change or retire a Conditional Access policy through a tenant-scoped, independently approved plan.",
        "permission": "entra.conditional_access.modify",
        "impact": "critical",
        "approval_required": True,
        "target": "tenant",
        "fields": [
            {
                "key": "policy_identifier",
                "label": "Policy name or provider ID",
                "type": "text",
                "required": True,
                "placeholder": "Require compliant device or policy GUID",
            },
            {
                "key": "policy_operation",
                "label": "Policy operation",
                "type": "select",
                "required": True,
                "default": "report_only",
                "options": [
                    {"value": "report_only", "label": "Create or update in report-only mode"},
                    {"value": "enable", "label": "Enable an approved policy"},
                    {"value": "disable", "label": "Disable a policy"},
                    {"value": "remove", "label": "Remove a policy"},
                ],
            },
            {
                "key": "excluded_breakglass_reviewed",
                "label": "Emergency access review",
                "type": "select",
                "required": True,
                "default": "confirmed",
                "options": [
                    {"value": "confirmed", "label": "Emergency access accounts reviewed"},
                    {"value": "not_applicable", "label": "Not applicable to this policy"},
                ],
            },
        ],
        "rollback": "Restore the recorded policy JSON or return the policy to report-only mode, then validate emergency access and tenant sign-in impact before closing the change.",
        "steps": [
            "Confirm the tenant, exact policy and intended scope",
            "Review break-glass exclusions, report-only evidence and service impact",
            "Request independent approval with a linked change reference",
            "Apply through an authorised provider and retain the before/after policy evidence",
        ],
    },
    {
        "id": "create-user",
