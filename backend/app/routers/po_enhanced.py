...
async def generate_po_pdf(po_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # The return above is the canonical Nexus commercial-document output.
    ...
    # Header
    # Match the Nexus report/invoice document family: an authoritative dark
    # masthead, the customer-facing document type, then retained provenance.
    pdf.set_fill_color(11, 23, 42)
    ...
    pdf.set_xy(x_text, 5)
    pdf.cell(100, 5, "NEXUSMSP | PROCUREMENT RECORD")
    pdf.set_font("Helvetica", "B", 18)
    pdf.set_xy(x_text, 13)
    ...
    actor = (current_user.get("name") or current_user.get("email") or cn).encode('latin-1', 'ignore').decode('latin-1')
