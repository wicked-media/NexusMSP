"""Smart Invoice Engine — AI Draft, Payment Plans, Smart Reminders, Late Fees,
...
async def _ai_chat(session_id: str, system_msg: str):
    """Wrap emergentintegrations Claude Sonnet 4.5 for invoice AI features."""
...
# ╔══════════════════════════════════════════════════════════════════╗
# ║   1) AI DRAFT INVOICE — from tickets / time entries / contracts  ║
# ╚══════════════════════════════════════════════════════════════════╝
...
async def ai_draft_invoice(data: dict, current_user: dict = Depends(get_current_user)):
    ...
    # Build line items
    line_items = []
    # 1. Time entries → grouped per ticket
    by_ticket = {}
    ...
    if include_recurring:
        ...
        for ri in ris:
            for li in (ri.get("line_items") or []):
                ...
                line_items.append({
                    "description": f"{li.get('description', '')} — {ri.get('description', 'Recurring')}",
...
# ╔══════════════════════════════════════════════════════════════════╗
# ║   4) SMART REMINDERS — escalating 3/7/14/30 day tone              ║
# ╚══════════════════════════════════════════════════════════════════╝
...
async def smart_reminder(invoice_id: str, data: dict, current_user: dict = Depends(get_current_user)):
    ...
    return {"stage": stage, "subject": f"Reminder: Invoice {invoice.get('invoice_number')} — {invoice.get('currency', 'AUD')} {invoice.get('total')}", "body": body, "days_overdue": max(0, age)}


# ╔══════════════════════════════════════════════════════════════════╗
# ║   5) REISSUE FROM PRIOR PERIOD                                    ║
# ╚══════════════════════════════════════════════════════════════════╝
...
# ╔══════════════════════════════════════════════════════════════════╗
# ║  11) LATE-FEE POLICY (per-client / global)                        ║
# ╚══════════════════════════════════════════════════════════════════╝
