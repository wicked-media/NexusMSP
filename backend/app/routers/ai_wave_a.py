...
Wave A AI differentiators:
  1. Ticket Auto-Co-pilot  — summarize thread / suggest next step / draft reply
  2. Explain This Error    — plain-English diagnosis + remediation
  3. Morning Standup Digest — AI brief of overnight events
"""
...
async def _llm_complete(system_msg: str, user_msg: str, session_prefix: str = "wave-a") -> str:
    """Single-shot completion using Emergent LLM key. Returns text or a friendly fallback."""
...
async def ticket_copilot(
    ...
    thread_lines = [
        f"[{(c.get('author') or 'user')} • {(c.get('created_at') or '')[:16]}] "
...
async def standup_digest(hours: int = 12, current_user: dict = Depends(get_current_user)):
    ...
    ai_brief = None
    if out.startswith("__AI_NOT_CONFIGURED__"):
        ai_brief = "AI briefing unavailable (EMERGENT_LLM_KEY not configured)."
...
# ──────────────────────────────────────────────────────────────────────────────
# Settings for digest delivery (email/SMS/banner) — persisted per-admin
# ──────────────────────────────────────────────────────────────────────────────
